...
- **Ensure mDNS is Enabled**: Modern browsers support mDNS to protect your local IP—double-check it’s enabled. On Chrome navigate to `chrome://flags#enable-webrtc-hide-local-ips-with-mdns` and Brave to `brave://flags#enable-webrtc-hide-local-ips-with-mdns` and ensure it's enabled. On Firefox navigate to `about:config` and ensure both `media.peerconnection.ice.no_host` and `media.peerconnection.ice.relay_only` are set to `True`. For other browsers and platforms instructions exist all over the web.
