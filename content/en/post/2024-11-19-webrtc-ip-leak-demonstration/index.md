...
date = 2024-11-19T19:35:00+01:00
...
WebRTC (Web Real-Time Communication) is a free, open-source technology that enables real-time communication between web browsers and mobile devices. By using WebRTC, developers can create seamless peer-to-peer (P2P) audio, video, and data connections without the need for additional plugins, custom software, or third-party extensions.
...
- [**STUN Servers:**](https://en.wikipedia.org/wiki/STUN) To establish a connection, a device uses a **STUN server** to discover its public IP and network configuration. This information is included in the SDP and shared with the peer, exposing both private and public IP addresses.

- [**TURN Servers (Optional):**](https://en.wikipedia.org/wiki/Traversal_Using_Relays_around_NAT)  If NAT or firewall settings block direct connections, a **TURN server** acts as a middleman, relaying data between peers. While this reduces the risk of exposing private IPs, it’s less efficient than direct connections. Please note that as the specification defines, the private IP would still get sent to the TURN server. So, if the malicious server and the TURN server cooperate it still leaks your IP.
...
Exploring the [Chrome metrics historical data](https://chromestatus.com/) reveals fascinating insights into WebRTC's usage. Back in 2018, WebRTC accounted for approximately **8% of total Chrome page loads**—likely translating to billions of loads per day. Notably, around **4% of total Chrome page loads** (or half of the WebRTC-related loads) involved calls to [SetLocalDescription](https://chromestatus.com/metrics/feature/timeline/popularity/1054), a critical step for initiating ICE candidate gathering.

In contrast, calls to [SetRemoteDescription](https://chromestatus.com/metrics/feature/timeline/popularity/1057), essential for establishing an actual connection, were only seen in **0.04% of total page loads**. This disparity reveals that a staggering **99.96% of these requests** were likely used for IP address gathering—an unintended and potentially malicious use of WebRTC's capabilities that it was never originally designed for!

## mDNS to the Rescue

Fortunately, it’s not all doom and gloom. Browser vendors have spent years addressing WebRTC IP leak vulnerabilities, and modern browsers have increasingly adopted [**mDNS host candidates**](https://en.wikipedia.org/wiki/Multicast_DNS)—a feature designed to anonymize local IP addresses during the ICE process.

mDNS anonymizes local IP addresses during the ICE process by replacing them with randomized hostnames, effectively preventing direct exposure of your private IP. While mDNS adoption and support varies, most major desktop browsers have implemented and enabled it by default, but on mobile it's still a major issue. For instance:
...
Even with mDNS enabled, IPv6 leaks remain a significant concern, as mDNS primarily addresses the exposure of local IPv4 addresses. When using a VPN, mDNS can protect your local IP from leaking, but your public IPv4 and IPv6 addresses may still be exposed unless additional protective measures are implemented.

While IPv4 with NAT generally offers more privacy by shielding internal devices behind a shared public IP address, IPv6 significantly weaken your privacy.

To clarify, IPv6 poses a significant concern when safeguarding your physical location is of utmost importance for several reasons:
...
- This direct accessibility increases the risk of exploitation, as devices may be exposed to potential attacks without the protective barrier that NAT provides.

**Tracking and Fingerprinting**:

If someone obtains your IPv6 address, they could use it to:
