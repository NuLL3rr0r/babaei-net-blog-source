...
OK, in order to configure our firewall we have to modify <code>/etc/rc.conf</code>. First, you shoud make sure no other firewall is running. Look for <code>pf_enable="YES"</code> or <code>ipfilter_enable="YES"</code> inside <code>/etc/rc.conf</code>. If you have any of them you shoud disable them by either setting their value to <code>"NO"</code> or remvoing them completely.
...
By default the above setup blocks all inbound connections on all ports for both TCP and UDP. <code>firewall_myservices</code> is a whiht-space separated list of ports. So, you have to address a specific port here if you want to allow inbound connection for that port. As you have noticed by now, instead of specifying port numbers I mentioned them by name in the <code>firewall_myservices</code> list. It's possible to either use port number or if a known or standard service uses a specific port, address it by name instead.
...
As you can see the equal port numbers are 25 and 465. Depending on how you configured your mail server (e.g. 25 for PLAIN and STARTTLS, 465 for TLS/SSL) you can pick one or both 25 or 465.

As you may have noticed already, I did mixed numbers and names in the <code>firewall_myservices</code> list. That's becuase I used non-defualt port instead of 22 for <code>ssh</code>. It is perfectly fine to mix numbers and names in the list.
...
This adjusts the number of logs per IP to 5. In order to make a permanent for future reboots of your server we should add it to <code>/etc/sysctl.conf</code>.
